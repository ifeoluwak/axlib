    ...
    return async () => {
        ...
        if (bodys?.data) {
            ...
            if (!thisTypeSourceFile) {
                ...
                const cwd = path.relative(`${config.apiPath}/${typeName}.ts`, `${config.typePath}/${typeName}.ts`);
                console.log('CWD', cwd);
                        ...
                        if (text.includes('typedApiWrapper') && text.includes(fn.name)) {
                            ...
                            fnDefinition.forEach((fnDef) => {
                                ...
                                if (prop) {
                                    ...
                                    pp.forEach((p) => {
                                        p.getDescendantsOfKind(SyntaxKind.Identifier).forEach((id) => {
                                            const idt = id.getText();
                                            if (['get', 'post', 'put', 'delete', 'patch'].includes(idt)) {
                                                p.setExpression(`${p.getExpression().getText().replace(idt, `${idt}<{ data: ${typeName} }>`)}`);
